...
class AuthIdManager extends APIModule
{
	...
	public function deleteExpiredAuthIds()
	{
		...
		for ($i = ($vals_len - 1); $i >= 0; $i--) {
			if ($vals[$i]['expires'] < $current_time) {
				AuthIdRecord::deleteByRecord($vals[$i]);
			}
		}
	}
	 ...
	 * @return bool true if authorization identifier deleted successfully, otherwise false
	 */
	 ...
	 * @param int $expires expiration date in UNIX timestamp format for authorization identifier
