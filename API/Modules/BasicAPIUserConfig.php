...
class BasicAPIUserConfig extends BasicUserConfig implements IUserConfig
{
	 ...
	 */
	public const USERS_FILE_NAME = 'Bacularis.API.Config.bacularis';
	...
	public function validateUsernamePassword($username, $password, $check_conf = true)
	{
		...
		if ($username && $password) {
			$user = $this->getModule('basic_config')->getConfig($username);
			$api_user = $this->getUserCfg($username);
			if (count($api_user) > 0 && (count($user) > 0 || !$check_conf)) {
				$mod = $this->getModule('crypto')->getModuleByHash($api_user['pwd_hash']);
				if (is_object($mod)) {
					$valid = $mod->verify($password, $api_user['pwd_hash']);
				}
			}
		}
		return $valid;
