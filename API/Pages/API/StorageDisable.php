<?php
/*
 * Bacularis - Bacula web interface
 *
 * Copyright (C) 2021-2026 Marcin Haba
 *
 * The main author of Bacularis is Marcin Haba, with contributors, whose
 * full list can be found in the AUTHORS file.
 *
 * You may use this file and others of this release according to the
 * license defined in the LICENSE file, which includes the Affero General
 * Public License, v3.0 ("AGPLv3") and some additional permissions and
 * terms pursuant to its AGPLv3 Section 7.
 */

use Bacularis\API\Modules\BaculumAPIServer;
use Bacularis\Common\Modules\Errors\StorageError;

/**
 * Disable storage endpoint.
 *
 * @author Marcin Haba <marcin.haba@bacula.pl>
 * @category API
 */
class StorageDisable extends BaculumAPIServer
{
	public function set($id, $params)
	{
		$cli = null;
		if ($id > 0) {
			$storage = $this->getModule('storage');
			$cli = $storage->getStorageById($id);
		}

		$storage_name = null;
		$bconsole = $this->getModule('bconsole');
		$result = $bconsole->bconsoleCommand(
			$this->director,
			['.storage'],
			null,
			true
		);
		if ($result->exitcode === 0) {
			if (is_object($cli) && in_array($cli->name, $result->output)) {
				$storage_name = $cli->name;
			}
		}
		if (is_null($storage_name)) {
			$this->output = StorageError::MSG_ERROR_STORAGE_DOES_NOT_EXISTS;
			$this->error = StorageError::ERROR_STORAGE_DOES_NOT_EXISTS;
			return;
		}

		$cmd = ['disable', 'storage="' . $storage_name . '"'];
		$result = $bconsole->bconsoleCommand(
			$this->director,
			$cmd
		);
		if ($result->exitcode === 0) {
			$this->output = $result->output;
			$this->error = StorageError::ERROR_NO_ERRORS;
		} else {
			$emsg = sprintf(
				' Error: %s, ExitCode: %d.',
				var_export($result->output, true),
				$result->exitcode
			);
			$this->output = StorageError::MSG_ERROR_WRONG_EXITCODE . $emsg;
			$this->error = StorageError::ERROR_WRONG_EXITCODE;
		}
	}
}
